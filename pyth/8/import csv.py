...
class CSVProcessor:
    def __init__(self, filename): #создает файл и загружает инф
        self.filename = filename
        self.data = self.load_csv()

    def load_csv(self):      #загружает cvs dqtq 
        with open(self.filename, 'r') as file:
            reader = csv.reader(file)
            data = list(reader)
        return data

    def show(self, output_type='top', num_rows=5, separator=','): #показывае опр колво строк из cvs dqtq
        header = self.data[0]
        rows = self.data[1:]
    ...
    def del_nan(self): #убыраем пустые стр
        header = self.data[0]
        rows = self.data[1:]
        self.data = [header] + [row for row in rows if all(field for field in row)]

    def make_ds(self): #создаем сплит и сохраняем 
        header = self.data[0]
        rows = self.data[1:]
